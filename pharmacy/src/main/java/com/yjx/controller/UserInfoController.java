...
import com.yjx.service.AuthService;
...
public class UserInfoController {
    ...
    /**
     * 更改头像,保存到 img/avatar/ 下
     *
     * @param file 文件
     * @return Result
     */
    @PostMapping("/upload/avatar/{username}")
    public Result uploadCarouselPicture(MultipartFile file, @PathVariable String username) {
        ...
        if (size / mb / mb > maxSize) {
            ...
            try {
                ...
                // 更换头像路径
                UpdateWrapper<UserInfo> updateWrapper = new UpdateWrapper<>();
                updateWrapper.eq("user_id", user.getId()).set("avatar", contentUrl);
                boolean updateRes = userInfoService.update(new UserInfo(), updateWrapper);
                log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Calendar.getInstance().getTime()) + "上传头像" + contentUrl);
                Map<String, Object> map = new HashMap<>(5);
                map.put("avatar", contentUrl);
