...
public class AccountController {
    ...
    public Result registerAccount(@RequestBody RegisterDto registerDto) {
        String emailCode = (String) redisTemplate.opsForValue().get("emailCode" + registerDto.getEmail());
        // 1.判断邮箱是否给用户发送了验证码
        if (code == null) {
            log.info("请先获取验证码");
            return Result.error().message("请先获取验证码");
        ...
        // 没有过期，进行匹配
        else if (!registerDto.getEmailCode().toLowerCase().equals(emailCode.toLowerCase())) {
            // 匹配失败
            log.info("验证码错误");
            return Result.error().message("验证码错误");
        }
        // 匹配成功
            ...
            User queryUser = userService.getOne(new QueryWrapper<User>().eq("username", registerDto.getUsername()));
            // 如果这个用户名已经存在
            if (queryUser != null) {
                log.info("用户名已存在");
